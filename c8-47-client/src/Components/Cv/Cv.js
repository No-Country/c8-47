...
export const Cv = () => {
  ...
  return (      
    ...
    <div className='   w-[220px] h-[220px] bg-blend-overlay grid grid-rows-2 grid-cols-2 hover:bg-gray-700 gap-0.5 divide-y divide-x  bg-center bg-no-repeat bg-cover' style={{ backgroundImage: `url(${curri})` }} >
        {parts.map((part,i)=> <>
            <div key={i} className=" hover:bg-gray-600 flex items-center justify-center flex-col text-center gap-2 cursor-pointer text-white">
            {part.icon} 
            <p>{part.title}</p>
        </div>
